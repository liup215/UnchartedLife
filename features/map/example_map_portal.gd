...
func _activate_portal(player: Node2D):
	if target_map_id.is_empty():
		GameLogger.warn("map", "Portal: No target map set!")
		return
	
	# Check if target map exists
	var target_map = MapManager.get_map_data(target_map_id)
	if not target_map:
		GameLogger.warn("map", "Portal: Target map '%s' not found!" % target_map_id)
		return
	...
	if MapManager.switch_to_map(target_map_id, spawn_pos):
		...
		GameLogger.info("map", "Portal: Teleported to %s at %s" % [target_map_id, spawn_pos])

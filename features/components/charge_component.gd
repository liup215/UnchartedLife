...
func start_heavy_charge():
	if not is_charging_heavy:
		...
		GameLogger.debug("charge", "Started heavy attack charging from level %d (%.1f%%)" % [current_charge_level, current_charge_progress])
...
func stop_heavy_charge() -> float:
	...
	GameLogger.debug("charge", "Released heavy attack with charge level: %.2f (level %d, %.1f%% progress)" % [effective_charge, current_charge_level, current_charge_progress])
...
func _update_heavy_charge(delta: float):
	...
	while current_charge_progress >= progress_per_level and current_charge_level < max_charge_level:
		...
		GameLogger.debug("charge", "Level up! Now at level %d" % current_charge_level)
...
func add_light_attack_charge(progress_amount: float = 20.0):
	...
	while current_charge_progress >= progress_per_level and current_charge_level < max_charge_level:
		...
		current_charge_level += 1
		charge_level_up.emit(current_charge_level)
		GameLogger.debug("charge", "Light attack level up! Now at level %d" % current_charge_level)
	...
	charge_changed.emit(current_charge_level, current_charge_progress, max_charge_level)
	GameLogger.debug("charge", "Light attack charge accumulated: level %d -> %d (%.1f%% progress)" % [old_level, current_charge_level, current_charge_progress])
...
func reset_charge():
	if current_charge_level > 0 or current_charge_progress > 0:
		...
		current_charge_progress = 0.0
		charge_changed.emit(current_charge_level, current_charge_progress, max_charge_level)
		GameLogger.debug("charge", "Charge reset to 0")

...
func is_met(context: Node) -> bool:
	...
	if has_enough:
		GameLogger.debug("eca", "ConditionHasItem: Player has %d of '%s' (required: %d)" % [total_count, item_id, required_count])
	else:
		GameLogger.debug("eca", "ConditionHasItem: Player has only %d of '%s' (required: %d)" % [total_count, item_id, required_count])

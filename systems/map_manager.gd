...
func register_map(map_data: MapData) -> void:
	if map_data and not map_data.map_id.is_empty():
		...
		GameLogger.debug("map", "Registered map '%s'" % map_data.map_id)
	else:
...
func switch_to_map(map_id: String, spawn_position: Vector2 = Vector2.ZERO) -> bool:
	...
	GameLogger.info("map", "Switched to map '%s' at position %s" % [map_id, target_spawn])
	...
	if map_parent:
		if current_map_data.use_chunk_loading:
			...
			GameLogger.info("map", "Initialized chunk loading for map '%s'" % map_id)
			...
			GameLogger.info("map", "Loaded full map scene for map '%s'" % map_id)
	else:
...
func _unload_all_chunks():
	...
	GameLogger.debug("map", "Unloaded all chunks and map scenes")

func _load_full_map_scene(scene_path: String):
	var map_scene_resource = load(scene_path)
	if map_scene_resource:
		var instance = map_scene_resource.instantiate()

	# If the scene is a Control (UI), wrap it in a CanvasLayer so it renders properly on screen
		if instance is Control:
			var canvas_layer = CanvasLayer.new()
			canvas_layer.name = "MapUILayer"
			canvas_layer.add_child(instance)
			current_map_scene = canvas_layer
			GameLogger.debug("map", "Loaded UI map scene wrapped in CanvasLayer")
		else:
			current_map_scene = instance

		map_parent.add_child(current_map_scene)
		GameLogger.info("map", "Loaded full map scene from %s" % scene_path)
	else:
		GameLogger.error("map", "Failed to load map scene from %s" % scene_path)
...
func _load_chunk(coords: Vector2i):
	...
	if chunk_scene:
		...
		GameLogger.debug("map", "Loaded chunk: %s" % str(coords))
...
func _unload_chunk(coords: Vector2i):
	if loaded_chunks.has(coords):
		...
		GameLogger.debug("map", "Unloaded chunk: %s" % str(coords))
...
func load_data(data: Dictionary) -> void:
	...
	if data.has("loaded_chunk_coords") and not data["loaded_chunk_coords"].is_empty():
		...
		GameLogger.debug("map", "Will restore %d chunks when map_parent is set" % chunks_to_restore.size())
...
func reset_for_new_game() -> void:
	...
	current_map_id = DEFAULT_MAP_ID
	current_map_data = available_maps.get(DEFAULT_MAP_ID)
	GameLogger.debug("map", "Reset for new game")
